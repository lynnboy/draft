...
\rSec1[depr.static]{static keyword}

\pnum
The use of the \tcode{static} keyword is deprecated when declaring objects in
namespace scope (see~\ref{basic.scope.namespace}).

\rSec1[depr.access.dcl]{Access declarations}

\pnum
Access declarations are deprecated (see~\ref{class.access.dcl}).

\rSec1[depr.string]{Implicit conversion from const strings}

\pnum
The implicit conversion from const to non-const qualification for string
literals~(\ref{conv.array}) is deprecated.

\rSec1[depr.c.headers]{Standard C library headers}
...
the corresponding
\tcode{c\textit{name}}
header is also placed within the namespace scope of the namespace
\tcode{std} and is followed by an
explicit \grammarterm{using-declaration}~(\ref{namespace.udecl}).
\indextext{namespace}%
...
The header
\tcode{<stdlib.h>}
makes these available also in the global namespace,
...
that satisfies the requirements of type \tcode{OFF_T}
(\ref{lib.iostreams.types}).
...
that satisfies the requirements of type \tcode{POS_T}
(\ref{lib.iostreams.forward}).

\pnum
An implementation may provide the following additional member function, which has the
effect of calling
\tcode{sbumpc()}~(\ref{lib.streambuf.pub.get}):
...
specified in clause~\ref{lib.input.output}:
...
in clause~\ref{lib.input.output}.
...
\tcode{INT_MAX}
is defined in
\tcode{<climits>}~(\ref{lib.support.limits}).
...
and
\tcode{strmode \& frozen == 0}.~(_{lib.strstreambuf.virtuals}_ describes how a dynamically allocated array object is freed.)
\end{itemdescr}
...
an \ntbs whose first element is designated by \tcode{s}.
The constructor is
\tcode{strstreambuf(s,n,s+std::strlen\brk(s))}.
